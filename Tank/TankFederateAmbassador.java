 ...
 * {@link TankFederate}. It will log information about any callbacks it
...
public class TankFederateAmbassador extends NullFederateAmbassador
{
	    ...
	    throws FederateInternalError
	{
		...
		for( AttributeHandle attributeHandle : theAttributes.keySet() )
		{
			...
			if( attributeHandle.equals(federate.targetPositionHandle) )
			{
				builder.append( attributeHandle );
				...
				federate.calculateTarget(federate.targetPosition);
			}
			else if( attributeHandle.equals(federate.targetDirectionHandle) )
			{
				...
				builder.append( ", attributeValue=" );
				HLAinteger32BE targetDirect = new HLA1516eInteger32BE();
				try {
					targetDirect.decode(theAttributes.get(attributeHandle));
				} catch (DecoderException e) {
					e.printStackTrace();
				}
				builder.append( targetDirect.getValue() );
				federate.targetDirection = targetDirect.getValue();
			}
			else
			{
				builder.append( attributeHandle );
	    ...
	    throws FederateInternalError
	{
		...
		for( ParameterHandle parameter : theParameters.keySet() )
		{
			if(parameter.equals(federate.wasHitRemoveBulletHandle))
			{
				...
				federate.wasHitLast = countValue;
		...
		federate.newShoot(federate.targetPosition, federate.targetDirection, federate.bulletPositionLast, federate.wasHitLast);
